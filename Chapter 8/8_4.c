...
int _fillbuf(FILE *fp) {
    if(fp->flag!=_READ&&fp->flag!=_ERR&&fp->flag!=_EOF)
        return EOF;
...
int _flushbuf(int x, FILE *fp) {
    ...
    if(fp->base==NULL) {
        ...
        write(stdout->fd,fp->base,((fp->ptr)-(fp->base))*sizeof(char));
    }
    fp->ptr=fp->base;
    if(x!=EOF)
        *fp->ptr++=x;
...
long fseek(FILE *p, long offset, int origin) {
    return lseek(p->fd,offset,origin);
}
